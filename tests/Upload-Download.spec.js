...
test ('Upload download excel validation',async ({page})=>{
        const texttoChange = "Apple";
        const textToReplace = "Iphone";

        await page.goto("https://rahulshettyacademy.com/upload-download-test/");//go to that page 
        const downloadPromise = page.waitForEvent('download');// it will wait till the file is complete download
        await page.getByRole('button', { name: 'Download' }).click();//click on download button
        await downloadPromise;//again wait for download just for double check
        await writeExcelTest(texttoChange, textToReplace, "C:/Users/Pankaj/Downloads/download.xlsx"); // calling that function----> just by enter the parameters value  ("text we search","text we replace","file path of that file")
        await page.locator("#fileinput").click();//click on choose file option to upload
